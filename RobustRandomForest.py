...
class RobustRandomForest:
    ...
    def fit(self, X, y, alpha = 10):
        ...
        if self.robustness == 'lowess':
            self.oob_lambda = self.__oob_lambda(X, y, alpha)
    ...
    def predict(self, samples):
        ...
        prediction = (delayed(tree_i.predict)(samples[tree_i_features])for (tree_i, tree_i_features) in self.estimators_)
        ...
        if self.regression:
            if self.robustness == 'huber':
                ...
                omega = self.__compute_rf_weights(self.X, samples)
                forest_prediction = [np.sum(self.y * omega[i]) for i in range(samples.shape[0])]

            elif self.robustness == 'lowess':
                omega = self.__compute_rf_weights(self.X, samples)
                forest_prediction = [np.sum(self.oob_lambda * self.y * omega[i]) for i in range(samples.shape[0])]
    ...
    def __weights_optimization_tukey(self, trees_predictions):
        ...
        training_responses = self.y
        #training_responses = self.predict(self.X)
        forest_prediction = trees_predictions.mean(axis=0)
        i = 0
        for _ in range(10):
